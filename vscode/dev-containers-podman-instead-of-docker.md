...
**The direction this is heading, stated conservatively**: the devcontainer and the EE are converging on a clean division rather than staying two unrelated things — the devcontainer standardizes *authoring*, the EE standardizes *what actually runs in CI and production*, and the two increasingly share a base toolchain (`ansible-dev-tools`) without the production image carrying every editor extension and test dependency the devcontainer has. One practice worth adopting regardless of which side of that line you're on: promote and pin EE images by digest (`image@sha256:...`) rather than a mutable tag like `latest`, for the same reason pinning any dependency by digest beats a floating tag anywhere else.
